	...
	<div id="right">
		...
		<div id="rightBody">
 		...
 		<path fill-rule="evenodd"
				...
				<a href="product_managing.do"> 상품 관리 </a>
			</div>
					...
					<form id="frmSearch">
						<select class="searchList" id="field" name="field">
							<option value="category"
								${ param.field eq "category" ? " selected='selected'" : "" }>카테고리명</option>
							<option value="id"
								${ param.field eq "id" ? " selected='selected'" : "" }>아이디</option>
						</select> 
						<span class="textBox" style="vertical-align: middle"> 
						<input type="text" id="keyword" name="keyword" class="keywordBox"
							placeholder="내용을 입력해주세요"
							value="${ param.keyword ne 'null' ? param.keyword : ''}" />
						</span>
						<button class="searchBtn">
							<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
								fill="#858585" class="bi bi-search" viewBox="0 0 16 16">
  			<path
					...
					<table class="table tableList">
						<thead>
							<tr id="top_title">
								...
								<th style="width: 200px; border-bottom: 1px solid #E5E5E5;">아이디
									( 닉네임 )</th>
						...
						<tbody>
							...
							<c:forEach var="product" items="${ productList }" varStatus="i">
								<tr onclick="boardDetail('${ product.pcode }')"
									...
									<td>${ i.count }</td>
									<td><c:out
											value="${ product.id } ( ${ product.nickname } )" /></td>
						...
						<c:choose>
							<c:when test="${empty param.page }">
								<c:forEach var="pn" begin="1" end="${pageEnd }" step="1">
									<a
										href="product_managing.do?page=${pn }&keyword=${param.keyword}&field=${param.field}">
										<c:if test="${pn == 1 }">
							...
							<c:when test="${pageEnd != 1 }">
								<c:forEach var="pn" begin="${pageStart }" end="${pageEnd }"
									step="1">
									<a
										href="product_managing.do?page=${pn }&keyword=${param.keyword}&field=${param.field}">
										<c:choose>
			...
			<c:if test="${ not empty param.keyword }">
				<div class="btnDiv">
					<a href="product_managing.do"> <input type="button"
						class="btnCss" value="목록" id="allList">
					</a>
				</div>
			</c:if>
		</div>
